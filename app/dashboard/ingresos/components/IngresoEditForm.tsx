...
interface Props {
    ingreso: Ingreso;
    onSave: (datos: Partial<Ingreso>) => Promise<void>;
    onClose: () => void;
}
        ...
        <form onSubmit={guardarIngreso} className="space-y-4">
            ...
            {exito && (
                <div className="p-3 bg-[#2A7933]/10 border border-[#2A7933]/30 rounded-lg flex gap-2">
                    ...
                    <p className="text-sm text-[#2A7933]">Ingreso actualizado</p>
                </div>
            )}

            {/* Cantidad */}
            <div className="space-y-3">
                <h3 className="text-xs font-semibold text-slate-700 uppercase">Datos del Ingreso</h3>
                ...
                <div>
                    <label className={labelStyles}>Bodega</label>
                    <select
